...
Track 2 is now complete (WU2.0 through WU2.4 all shipped, all on `main`). The next unit per the improvement plan is **WU3.0** — a one-page ADR-style spike comparing a long-running asyncio worker against an ACA Job for the ingestion shape, cost-shape, and local-dev ergonomics. Output is a decision doc at `docs/adrs/0001-worker-shape.md`. Read `docs/4. services.md` first — the ingestion-worker non-responsibilities (no HTTP surface, no shared hot path with the API container) are the constraints the spike has to honour.
