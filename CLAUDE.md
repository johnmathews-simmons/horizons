...
5. `data/samples/README.md` — what the sample legal markdown is, how it was collected (`scripts/fetch_fixtures.py`), and the current 46-fixture inventory.
...
- `uv run python scripts/seed_curated_set.py` — seed `documents` + `document_poll_schedule` from `data/curated_set.yaml`; idempotent. Pass `--stage-synthetic-v2` to additionally stage every hand-authored v2 document under `data/samples/synthetic_v2/` (currently 8 pairs) (parks the corresponding `next_poll_at` at `2026-12-31` so the worker can't claim staged docs — see `journal/260605-fix-worker-staged-guard-and-env-validation.md`). `--dry-run` parses and aligns without writing.
- `uv run python packages/horizons-api/scripts/create_demo_accounts.py` — provision the three demo accounts (`demo-uk@demo.example.com`, `demo-eu@demo.example.com`, `admin-demo@demo.example.com`). Default run is create-or-rotate with a no-downgrade guard (refuses to overwrite a real credential with the bake-in default); `--reset` deletes and recreates. Override passwords via `HORIZONS_DEMO_{UK,EU,ADMIN}_PASSWORD`. Documented in `docs/runbooks/demo-accounts.md`.
...
- `scripts/reseed_aca.sh [--yes]` — laptop wrapper that dispatches the `horizons-dev-reseed-corpus` Container Apps Job to wipe + re-seed the staging corpus and rotate the demo accounts. Dry-run by default (prints the worker + Job image tags); `--yes` requires a typed-back confirmation. Needed because the `documents` table is append-only via trigger — editing `data/curated_set.yaml` and re-deploying does not update existing rows. Runbook: `docs/runbooks/reseed.md`. Implementation: `scripts/reseed_corpus.py` (in-container orchestrator) + `infra/modules/reseed-corpus-job.bicep` (Job definition).
...
- `gh workflow run infra-drift-check.yml` — manual `az deployment group what-if` against `horizons-nonprod`; opens a GH issue labelled `infra-drift` if the change set is non-empty. Also scheduled at 03:00 UTC nightly. Triggers are `schedule` + `workflow_dispatch` only (no push trigger).
