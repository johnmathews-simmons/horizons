...
The first end-to-end Azure deploy happened on 2026-06-06 after WU8.4. It surfaced an 8-bug ledger of Bicep ↔ application contract mismatches; all 8 are closed and the API + worker + SPA are now live in `horizons-nonprod`. The session retrospective + post-demo punch list lives at `journal/260606-deploy-pipeline-end-to-end.md` — start there for the current state of the deploy pipeline and any IaC drift to clean up post-demo.
