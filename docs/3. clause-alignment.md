...
This doc fleshes out principle 4 in `2. database-design.md`.
...
For clauses still unpaired, run a similarity match using one of the techniques in the next section, **constrained to non-crossing order**: if v1's clause at position *i* is paired with v2's clause at position *p(i)*, then for all *i < j*, *p(i) < p(j)*. This is the longest-common-subsequence constraint, scaled from "lines" to "clauses." Without it, the matcher would happily pair v1 clause 5 with v2 clause 100 just because the text overlaps — which is nearly always nonsense in ordered legal documents.
...
- Two clauses with very similar boilerplate content (recurring definitions, recurring penalty clauses) can be mis-paired. The monotonic constraint helps, but doesn't eliminate this.
...
- Default *k* for shingling. *k* = 5 is the usual choice for English prose; legal text with shorter sentences may want *k* = 3. Decide empirically once we have parsed corpus volume.
- Choice of MinHash signature size and LSH band parameters — depends on corpus scale at the time we build this.
- Whether to keep per-clause MinHash signatures as a database column (cheap, useful for cross-document near-duplicate detection later) or as an ingestion-time-only artefact.
- How to surface alignment confidence in the API response — as a raw float, a bucketed label (`high|medium|low`), or both.
