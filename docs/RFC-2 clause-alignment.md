...
The two numbers tell different stories. As of 2026-06-07, the same default `TuningConfig` produces macro-F1 = 0.97 on the synthetic-mutation suite and macro-F1 = 0.77 on the gold suite — recall is the stronger axis on both (≈0.90 on the gold suite, ≈0.97 on the mutation suite) but precision is the bottleneck on the gold suite (≈0.70) because realistic insertions and removals push sibling paragraphs down and the aligner correctly-but-noisily emits each shifted paragraph as a MOVED event. Report both numbers when tuning; a drop in either is a regression worth explaining. See `journal/260607-alignment-gold-suite.md` for the original per-fixture breakdown and the two collapsing-noise mitigations to consider post-demo, and `journal/260607-gold-file-regenerated-for-stale-ref-fixes.md` for the c95911f gold update.
